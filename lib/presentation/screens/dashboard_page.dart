...
class _DebugButton extends StatelessWidget {
  const _DebugButton({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return IconButton(
      icon: const Icon(Icons.bug_report, color: Color(0xFF009688)),
      onPressed: () => _showDebugDialog(context),
    );
  }

  void _showDebugDialog(BuildContext context) async {
    final fcmService = FCMHttpService();

    showDialog(
      context: context,
      builder:
          (context) => const AlertDialog(
            title: Text('Debug FCM Tokens'),
            content: SizedBox(
              width: double.maxFinite,
              child: _DebugTokensWidget(),
            ),
          ),
    );
  }
...
class _DebugTokensWidget extends StatefulWidget {
  const _DebugTokensWidget({Key? key}) : super(key: key);

  @override
  State<_DebugTokensWidget> createState() => _DebugTokensWidgetState();
}

class _DebugTokensWidgetState extends State<_DebugTokensWidget> {
  ...
  Future<void> _loadTokens() async {
    final fcmService = FCMHttpService();
    final userTokens = await fcmService.getCurrentUserTokens();
  ...
  Future<void> _forceTokenRefresh() async {
    ...
    try {
      ...
      final newToken = await notificationService.forceTokenRefresh();

      if (newToken != null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Token FCM aggiornato con successo!'),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Errore nell\'aggiornamento del token FCM'),
    ...
    return Column(
      ...
      children: [
        Row(
          ...
          children: [
            Text(
              'Token FCM trovati: ${tokens.length}',
        ...
        if (tokens.isEmpty)
          const Column(
            children: [
              ...
              SizedBox(height: 8),
              Text(
                'I token potrebbero non essere stati salvati correttamente',
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text('Piattaforma: ${token['platform']}'),
                        Text('Attivo: ${token['isActive'] ? 'Sì' : 'No'}'),
                        Text('Creato: ${token['createdAt']}'),
                        Text('Ultimo uso: ${token['lastUsed']}'),
                      ],
                    ),
                  ),
                ),
